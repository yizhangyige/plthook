...
PLTHook 是一个跨平台的动态链接库hook工具，支持 Linux、macOS、Windows 和 Android 等多个平台。本项目现在提供了完整的双重脚本体系，包括原有的 PowerShell/Batch/Shell 脚本和新增的功能增强型 Python 脚本版本。
...
- **传统脚本**：保留原有的 PowerShell、Batch 和 Shell 脚本，确保向后兼容性
...
| 原版脚本 | Python 版本 | 功能增强 | 跨平台支持 |
|----------|-------------|----------|------------|
| build_windows.ps1 | build_windows.py | ✓ VS自动检测 | ✓ |
| build_android.ps1 | build_android.py | ✓ 多架构并行 | ✓ |
| build.ps1 | build.py | ✓ 统一接口 | ✓ |
| build.sh | build_linux.py | ✓ 现代化构建 | ✓ |
| test_android.ps1 | test_android.py | ✓ 设备管理 | ✓ |
| test/run-test.bat | test/run_test.py | ✓ 增强测试 | ✓ |
| test/run_tests_windows.bat | test/run_tests_windows.py | ✓ 并行测试 | ✓ |
| test/uclibc-test.sh | test/uclibc_test.py | ✓ 跨平台测试 | ✓ |
| test/android/run_tests.sh | test/android/run_tests.py | ✓ 设备测试 | ✓ |
| test/build_memory_monitor.ps1 | test/build_memory_monitor.py | ✓ 工具集成 | ✓ |
